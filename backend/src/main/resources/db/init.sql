...
CREATE TABLE IF NOT EXISTS `competitors` (
    ...
    `features`      JSON                                     COMMENT '功能支持矩阵: {"feature_name":"yes"|"partial"|"no"}',
    `reviews`       JSON                                     COMMENT '用户评论摘要数组',
    `strengths`     JSON                                     COMMENT 'AI 总结的优势列表',
    `weaknesses`    JSON                                     COMMENT 'AI 总结的劣势列表',
    `needs_user_input` TINYINT(1)  NOT NULL DEFAULT 0       COMMENT 'AI 不熟悉该竞品时为 1, 提示用户补充',
    `created_at`    DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`    DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_project_id` (`project_id`),
...
-- ============================================================
-- 6.1 竞品对比分析报告表 (项目级聚合数据)
-- 每个项目一行, 存储项目级矩阵特征列表、空白机会标记、AI 差异化建议
-- 单个竞品的支持度仍存在 competitors.features 中, 此表只放跨竞品的聚合视图
-- ============================================================
